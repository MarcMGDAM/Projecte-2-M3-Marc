...
public class Client {
    ...
    public void modificarClients() throws SQLException, NoSuchAlgorithmException {

        ConnexioBD con = new ConnexioBD();
        con.conectarBD();
        Scanner teclat = new Scanner(System.in);

        System.out.println("Escriu el DNI del client que vol modificar: ");
        String dni = teclat.next();

        // Primer seleccionarem les dades de tots els productes
        String consultaSocis = " SELECT * FROM socis WHERE DNI =" + dni;
        PreparedStatement dades = con.connexioBD.prepareStatement(consultaSocis);

        ResultSet rs = dades.executeQuery();

            // Agafem els atributs dels clients i li passarem unes variables sobre les dades
            // que volem deixar modificar al client
            Telefon mobil;
            Email correu;
            String usuari;
            String contrasenya;
            String condicio_fisica;
            Boolean comunicacio_comercial;
            // Iban compte_bancari;
            ...
            if (resposta.equals("si")) {
                ...
                // sentencia.setTelefon(1, telefon.getTelefon());
            }

            System.out.println(
                    "Si vol modificar el correu electronic del client escriu 'si', però si no vol modificar-ho escriu 'no'");
            ...
            System.out.println(
                    "Si vol modificar el correu electronic del client escriu 'si', però si no vol modificar-ho escriu 'no'");
            String resposta4 = teclat.nextLine();
            if (resposta4.equals("si")) {
            ...
            System.out.println(
                    "Si vol modificar el correu electronic del client escriu 'si', però si no vol modificar-ho escriu 'no'");
            String resposta5 = teclat.nextLine();
            if (resposta5.equals("si")) {
            ...
            System.out.println(
                    "Si vol modificar el correu electronic del client escriu 'si', però si no vol modificar-ho escriu 'no'");
            String resposta6 = teclat.nextLine();
            if (resposta6.equals("si")) {
    ...
    public Client modificarClientBD() throws SQLException {
        ...
        // Demanem el DNI
        String actualitzar = "UPDATE scois SET mobil = ?, email= ?, usuari= ?, contrasenya= ?, condicio_fisica= ?, comunicacio_comercial= ?, compte_bancari= ? where DNI = " + dni;

        try {
            PreparedStatement ps = con.connexioBD.prepareStatement(actualitzar);
            cargarDadesDelClient(ps);
            ps.executeUpdate();
        } catch (Exception ex) {
            System.out.println("Error al crear l'usuari" + this.nom);
